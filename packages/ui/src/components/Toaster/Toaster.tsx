...
const ToasterImpl: FC<ToasterProps> = ({
  ...
  closeButton = false,
  offset,
  mobileOffset = { bottom: '120px', top: '32px' },
}) => {
  return (
    <SonnerToaster
      ...
      toastOptions={{
        ...
        classNames: {
          toast:
            'bg-background text-foreground border-border border-(length:--border-width) font-bold shadow-shadow rounded-md text-sm flex items-center gap-2 p-4 w-80 [&:has(button)]:justify-between select-none z-50',
          description: 'font-normal',
          actionButton:
            'font-normal border-(length:--border-width) text-sm h-6 px-2 bg-primary text-foreground border-border rounded-md shrink-0',
          cancelButton:
